...
def on_enter(event):
    ...
    file_name = find_file("C:\\Users\Group-test\\Desktop\\Debug22.02.17\\烧录软体", model_name,
...
def uiAutomationAll300G(file_name=""):
    try:
        ...
        # if not_found_dialog.Exists(3):
        #     print("Job File find fail!!!!")
        #     not_found_dialog.ButtonControl(searchDepth=5, Name="确定").Click()
        #     jobFile.Disappears(1)
        #     jobFile.GetWindowPattern().Close()
        # else:
...
def uiAutomationBurn():
    try:
        window = auto.WindowControl(searchDepth=1, Name=" 群沃自动烧录系统")
        if not window.Exists():
